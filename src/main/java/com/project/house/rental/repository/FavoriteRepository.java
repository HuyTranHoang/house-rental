...
public interface FavoriteRepository extends JpaRepository<Favorite, FavoritePrimaryKey> {
    ...
    @Query("SELECT f FROM Favorite f WHERE f.favoritePrimaryKey.propertyId = :property AND f.isDeleted = false")
    List<Favorite> findByPropertyIdWithFilter(long userId);
}
